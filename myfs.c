	...
	if (found) {
		...
		while(remain > 0 && num <= total_devices){
			if (num == 0) {
				...
				res = read(fd, buffer2, toread);
				memcpy(buf, buffer2, toread);
				write(fd3, buffer2, toread);
				if (res == -1){
					pthread_mutex_lock(&m);
    					reading--;
    					pthread_cond_broadcast(&turn);
...
dprintf(fd2, "netoffset:%zu\n", netoffset);
				...
				for(int i = 0; i < 4; i++){
					arr[i] = 0xFF & rem;
					rem = rem >> 8;
					write(client_fd[num], (char*)arr+i, 1);
				}
				write(client_fd[num], (char*)&netoffset, sizeof(uint32_t));
				memset(buffer, 0, 1024);
				size_t toread;
				read(client_fd[num], (char*)&toread, sizeof(size_t));
dprintf(fd2, "toread:%zu\n", toread);
				int r = read(client_fd[num], buffer2, toread);
dprintf(fd2, "read returned:%d\n", r);
dprintf(fd2, "buffer2 strlen:%zu\n", strlen(buffer2));
				memcpy(buf+written, buffer2, toread);
				write(fd3, buffer2, toread);
				remain -= r;
				written += r;
//write(fd2, buffer, written);
